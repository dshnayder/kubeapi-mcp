...
description: Provides guidance for using the kubeapi-mcp tool with Cursor.
alwaysApply: true
---

# Kubernetes MCP Tool Instructions

This rule provides context for using the kubeapi-mcp tool within Cursor.

`

// CursorMCPExtension installs the kubeapi-mcp server as a Cursor MCP extension
func CursorMCPExtension(opts *InstallOptions) error {
	...
	// Create the rules directory and kubeapi-mcp.mdc file
	rulesDir := filepath.Join(mcpDir, "rules")
	if err := os.MkdirAll(rulesDir, 0755); err != nil {
		return fmt.Errorf("could not create rules directory: %w", err)
	}

	// Create the kubeapi-mcp.mdc rule file with custom heading and GEMINI.md content
	ruleContent := append([]byte(cursorRuleHeader), GeminiMarkdown...)

	rulePath := filepath.Join(rulesDir, "kubeapi-mcp.mdc")
	if err := os.WriteFile(rulePath, ruleContent, 0644); err != nil {
		return fmt.Errorf("could not write kubeapi-mcp rule file: %w", err)
	}
